...
router.post("/scanbarcode", authentication.shallPass, function(req, res, next) {
  ...
  if (req.body.q) {
    Product.findOne({ barcode: req.body.q })
      ...
      .exec(function(err, docs) {
        ...
        if (docs) {
          ...
          res.json({
            Message: "Couldn't find the barcode you are looking for"
          });
        }
      });
  } else {
    ...
    res.json({
      message:
        "Did you use Get instead of Post?. Also make sure you supply the proper parameters"
    });
  }
});
        ...
        Promise.all(prom)
          .then(function(docs) {
            ...
            if (docs) {
              ...
              res.json({
                Message: "Couldn't find the text you are looking for"
              });
            }
          })
          .catch(error => {
